...
class BookmarkedArticlesFragment : Fragment() {
    ...
    private val viewModel: BookmarkedArticlesViewModel by viewModels()
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        adapter = NewsAdapter { article ->
            val action = BookmarksFragmentDirections
                .actionBookmarksFragmentToNewsArticleFragment(article.url)
            findNavController().navigate(action)
        }
        binding.recyclerView.apply {
            layoutManager = LinearLayoutManager(context)
            adapter = this@BookmarkedArticlesFragment.adapter
        }
        ...
        viewLifecycleOwner.lifecycleScope.launch {
            viewModel.articles.collect { articles ->
                if (articles.isEmpty()) {
                    ...
                    binding.stateView.showEmpty(messageRes = R.string.state_empty_bookmark_articles)
                } else {
